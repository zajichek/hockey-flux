...
snapshot_at <- function(game_time_elapsed) {
  entity <- list(current = list(game_time_elapsed = game_time_elapsed))
...
test_that("exact period boundaries belong to the period that just ended", {
  ...
  end_regulation <- snapshot_at(3600)
...
test_that("a tied game continues after regulation", {
  entity <- transition_entity(period = 3, home_score = 2, away_score = 2)
  changes <- game_transition(
    entity,
    list(event_type = "period_end", time_next = 3600)
  )

  expect_equal(changes$game_time_elapsed, 3600)
  expect_null(changes$game_status)
})
...
test_that("a regulation lead finalizes the game at the period end", {
  entity <- transition_entity(period = 3, home_score = 3, away_score = 2)
  changes <- game_transition(
    entity,
    list(event_type = "period_end", time_next = 3600)
  )
...
test_that("the first overtime goal finalizes the game at its event time", {
  entity <- transition_entity(period = 3, home_score = 2, away_score = 2)
  goal_time <- 3600 + 15.25
  changes <- game_transition(
    entity,
    list(event_type = "goal_home", time_next = goal_time)
  )

  expect_equal(changes$game_time_elapsed, goal_time)
